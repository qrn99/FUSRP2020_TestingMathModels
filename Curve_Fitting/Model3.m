function Model3()
%MODEL3 Test the Delay Compartment Model with 4 breakfast glucose curves and 4
...
params(10)=k_xi;
params(11)=k_lambda;
params(12)=S_initial; %the initial glucose in stomach
params(13)=J_initial; %initial glucose in jejunum
params(14)=L_initial; %initiial glucose in ileum
params(15)=I_initial; %initial insulin concentration
params(16)=k_jc;
params(17)=k_cl;
params(18)=C_initial;
...
for i=1:size(data,2)
    ...
    %we set the maximum iterations of fminsearch to be 1600 to prevent
    %excessive runtime
    options = optimset('MaxIter',1600,'PlotFcns',@optimplotfval);
    %fun will now equal the current cost function, calculated as the sum of
    %the squared residuals
    fun=@(params)(Cost(params, gdata,3));
    ...
    %Set the initial values of S,J,L,G,I,C
    init_val=[x(12) x(13) x(14) gdata(1) x(15) x(18)];
    ...
    [t,y] = ode45(@(t,y) delayCompartmentODE(t,y,x),tspan,init_val);
...
title("Delay Compartment Model Breakfast")
...
%file
writematrix([storeBreakfastParam; storeLunchParam],'CompartmentDelayParameters.xlsx')
end
